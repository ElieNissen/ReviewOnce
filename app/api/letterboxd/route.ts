...
type LbFilm={id:string;tmdbId?:string;title:string;aliases?:string[];year:string;rating:number|null;watchedDate:string;review:string;sourceUrl:string;match:string};

function parseFilms(html:string,reviewOnly=false){
 const blocks=[...html.matchAll(/<li[^>]*class=["'][^"']*poster-container[^"']*["'][^>]*>([\s\S]*?)<\/li>/gi)].map(match=>match[0]);
 return blocks.map((block,index)=>{const title=attr(block,"data-item-name")||attr(block,"data-film-name")||attr(block,"alt"),year=attr(block,"data-item-year")||attr(block,"data-film-release-year")||title.match(/\((\d{4})\)$/)?.[1]||"",slug=attr(block,"data-item-slug")||attr(block,"data-film-slug"),tmdbId=attr(block,"data-tmdb-id")||attr(block,"data-film-tmdb-id")||undefined,ratingClass=block.match(/\brated-(\d+)\b/)?.[1];return{id:`lb-page-${slug||index}`,tmdbId,title:title.replace(/\s*\(\d{4}\)\s*$/,"").trim(),year,rating:ratingClass?Number(ratingClass):null,watchedDate:"",review:reviewOnly?"present":"",sourceUrl:slug?`https://letterboxd.com/film/${slug}/`:"",match:"complete"} as LbFilm}).filter(film=>film.title);
}
...
async function fetchPages(username:string,path:string,reviewOnly=false){
 ...
 const firstHtml=await first.text(),pages=Math.min(100,Math.max(1,...[...firstHtml.matchAll(/\/page\/(\d+)\//g)].map(match=>Number(match[1])))),results=parseFilms(firstHtml,reviewOnly);
 for(let page=2;page<=pages;page++){const response=await fetch(`${firstUrl}page/${page}/`,{headers});if(!response.ok)break;results.push(...parseFilms(await response.text(),reviewOnly))}
 return results;
}
...
export async function GET(request:Request){
 ...
 try{
  try{const [library,reviews]=await Promise.all([fetchPages(username,"films/by/date/"),fetchPages(username,"films/reviews/",true)]),reviewed=new Set(reviews.map(film=>`${normalize(film.title)}:${film.year}`)),movies=library.map(film=>({...film,review:reviewed.has(`${normalize(film.title)}:${film.year}`)?"present":""}));return Response.json({movies,total:movies.length,source:"full-profile",limited:false},{headers:{"cache-control":"public, max-age=300, s-maxage=1800"}})}
