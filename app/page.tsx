...
type NativeEvent={type:string;message?:string;username?:string;connected?:boolean;collection?:Film[];collectionCount?:number};
...
export default function Home(){
 ...
 const[nativeCollectionCount,setNativeCollectionCount]=useState<number|null>(null);
 ...
 useEffect(()=>{const inApp=/ReviewOnceAndroid\//i.test(navigator.userAgent);setAndroidApp(inApp);if(!inApp)return;const nativeWindow=window as typeof window&{__reviewOnceNativeEvent?:(payload:string)=>void};nativeWindow.__reviewOnceNativeEvent=(payload:string)=>{try{const event=JSON.parse(payload) as NativeEvent;if(event.type==="session"){setNativeConnected(Boolean(event.connected));if(typeof event.collectionCount==="number")setNativeCollectionCount(event.collectionCount);if(event.username){setLb(event.username);localStorage.setItem("senssync-lb",event.username)}}else if(event.type==="collection-progress"||event.type==="sync-progress"){setNotice(event.message||"Actualisation en cours…")}else if(event.type==="collection-complete"||event.type==="collection"){if(nativeTimer.current)window.clearTimeout(nativeTimer.current);const resolver=nativeResolve.current;resolver?.(event.collection||[]);nativeResolve.current=null;nativeReject.current=null;setNativeCollectionCount(event.collectionCount??event.collection?.length??0);if(event.type==="collection-complete"&&!resolver)setPendingComparison(true);if(event.message)setNotice(event.message)}else if(event.type==="collection-error"){if(nativeTimer.current)window.clearTimeout(nativeTimer.current);nativeReject.current?.(Error(event.message||"Lecture Letterboxd impossible."));nativeResolve.current=null;nativeReject.current=null}else if(event.type==="sync-error"){setError(event.message||"Synchronisation impossible.")}else if(event.type==="sync-complete"){setNotice(event.message||"Synchronisation terminée.")}}catch{setError("La réponse de l’application est illisible.")}};window.location.href="reviewonce://session";return()=>{delete nativeWindow.__reviewOnceNativeEvent;if(nativeTimer.current)window.clearTimeout(nativeTimer.current)}},[]);
 ...
 function readNativeCollection(refreshFromLetterboxd=false){return new Promise<Film[]>((resolve,reject)=>{nativeResolve.current=resolve;nativeReject.current=reject;nativeTimer.current=window.setTimeout(()=>{nativeResolve.current=null;nativeReject.current=null;reject(Error("La récupération prend trop de temps. Réessaie en gardant ReviewOnce ouvert."))},300000);window.location.href=refreshFromLetterboxd?`reviewonce://refresh?username=${encodeURIComponent(lb.trim())}`:"reviewonce://collection"})}
 ...
 function importLetterboxdExport(){setError("");setNotice("");window.location.href="reviewonce://export"}
 ...
 async function refresh(){
  ...
  try{
   ...
   const [sr,localMovies,lr]=androidApp?await Promise.all([srPromise,readNativeCollection(true),Promise.resolve(null)]):await Promise.all([srPromise,Promise.resolve(null),fetch(`/api/letterboxd?username=${encodeURIComponent(lb)}&_t=${nonce}`,{cache:"no-store"})]);
 ...
 const firstImport=Boolean(androidApp&&nativeConnected&&(nativeCollectionCount??0)===0);

 useEffect(()=>{if(pendingComparison&&configured){setPendingComparison(false);void refresh()}},[pendingComparison,nativeCollectionCount]);
 ...
 return <main className="app">
  ...
  <div className="content">
   {screen==="sync"&&<>
    ...
    {firstImport?<section className="firstImportCard"><div className="firstImportIcon"><Icon name="sync" size={26}/></div><h1>Récupère ta bibliothèque</h1><p>ReviewOnce va retrouver tes films, notes, critiques, dates et ta watchlist en arrière-plan.</p><div className="importSteps">{["Garde ReviewOnce ouvert quelques instants","La première récupération peut prendre quelques minutes","Les prochaines comparaisons seront préparées ici"].map((label,index)=><div key={label}><span>{index+1}</span><strong>{label}</strong></div>)}</div><button onClick={refresh} disabled={loading}><Icon name="sync" size={18}/>{loading?"Récupération en cours…":"Récupérer ma bibliothèque"}</button><small>Ta session et tes données restent sur cet appareil.</small>{notice&&<div className="syncNotice"><Icon name="alert" size={17}/><span>{notice}</span></div>}{error&&<><div className="syncError" role="alert"><Icon name="alert" size={17}/><span><strong>La récupération s’est interrompue</strong>{error}</span></div><button className="textAction" onClick={importLetterboxdExport}>Utiliser l’export officiel</button></>}</section>:<><section className="syncSummary"><div><span className="bigCount">{selectable.length}</span><div><h1>film{selectable.length>1?"s":""} à importer</h1><p>{selectable.length?"Choisis ceux à ajouter à Letterboxd.":"Actualise pour chercher les nouveaux films."}</p></div></div><button onClick={refresh} disabled={loading}><Icon name="sync" size={18}/>{loading?"Actualisation…":"Actualiser"}</button>{error&&<div className="syncError" role="alert"><Icon name="alert" size={17}/><span><strong>Un problème est survenu</strong>{error}</span></div>}{notice&&<div className="syncNotice"><Icon name="alert" size={17}/><span>{notice}</span></div>}{importReady&&<div className="importReady"><div><strong>Fichier prêt</strong><span>Le téléchargement est terminé.</span></div><button onClick={openLetterboxdImport}><Icon name="external" size={17}/>Ouvrir dans le navigateur</button></div>}</section>
