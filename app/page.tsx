...
export default function Home(){
 ...
 const firstImport=Boolean(androidApp&&nativeConnected&&nativeCollectionCount===0);
 const firstImportStage=notice.includes("journal")?3:notice.includes("critiques")?2:notice.includes("watchlist")?1:0;
 ...
 return <main className="app">
  ...
  <div className="content">
   {screen==="sync"&&<>
    ...
    {firstImport?<section className="firstImportCard"><div className={`firstImportIcon ${loading?"spin":""}`}><Icon name="sync" size={26}/></div><h1>Préparons ta bibliothèque</h1><p>ReviewOnce va lire une première fois tout ton compte Letterboxd. Cette étape peut prendre quelques minutes et ne sera pas répétée à chaque utilisation.</p><div className="importSteps">{["Films et notes","Watchlist","Critiques","Journal et dates"].map((label,index)=><div className={loading?(index<firstImportStage?"done":index===firstImportStage?"active":""):""} key={label}><span>{index<firstImportStage?<Icon name="check" size={14}/>:index+1}</span><strong>{label}</strong></div>)}</div><button onClick={refresh} disabled={loading}><Icon name="sync" size={18}/>{loading?"Récupération en cours…":"Récupérer ma bibliothèque"}</button>{loading&&<small>Garde ReviewOnce ouverte pendant cette première récupération.</small>}{notice&&loading&&<div className="importProgress">{notice}</div>}{error&&<div className="syncError" role="alert"><Icon name="alert" size={17}/><span><strong>La récupération s’est interrompue</strong>{error}</span></div>}</section>:<><section className="syncSummary"><div><span className="bigCount">{selectable.length}</span><div><h1>film{selectable.length>1?"s":""} à importer</h1><p>{selectable.length?"Choisis ceux à ajouter à Letterboxd.":"Actualise pour chercher les nouveaux films."}</p></div></div><button onClick={refresh} disabled={loading}><Icon name="sync" size={18}/>{loading?"Actualisation…":"Actualiser"}</button>{error&&<div className="syncError" role="alert"><Icon name="alert" size={17}/><span><strong>Un problème est survenu</strong>{error}</span></div>}{notice&&<div className="syncNotice"><Icon name="alert" size={17}/><span>{notice}</span></div>}{importReady&&<div className="importReady"><div><strong>Fichier prêt</strong><span>Le téléchargement est terminé.</span></div><button onClick={openLetterboxdImport}><Icon name="external" size={17}/>Ouvrir dans le navigateur</button></div>}</section>
