...
export default function Home(){
 ...
 function readNativeCollection(){return new Promise<Film[]>((resolve,reject)=>{nativeResolve.current=resolve;nativeReject.current=reject;nativeTimer.current=window.setTimeout(()=>{nativeResolve.current=null;nativeReject.current=null;reject(Error("La bibliothèque locale n’a pas pu être ouverte."))},30000);window.location.href="reviewonce://collection"})}
 ...
 async function refresh(){
  ...
  try{
   ...
   const [sr,localMovies,lr]=androidApp?await Promise.all([srPromise,readNativeCollection(),Promise.resolve(null)]):await Promise.all([srPromise,Promise.resolve(null),fetch(`/api/letterboxd?username=${encodeURIComponent(lb)}&_t=${nonce}`,{cache:"no-store"})]);
 ...
 return <main className="app">
  ...
  <div className="content">
   {screen==="sync"&&<>
    ...
    {firstImport?<section className="firstImportCard"><div className="firstImportIcon"><Icon name="sync" size={26}/></div><h1>Importe ta bibliothèque</h1><p>ReviewOnce utilise l’export officiel Letterboxd pour récupérer en une fois tes films, notes, critiques, dates et ta watchlist.</p><div className="importSteps">{["Ouvre la page de données Letterboxd","Appuie sur Export Your Data","ReviewOnce récupère et compare le fichier"].map((label,index)=><div key={label}><span>{index+1}</span><strong>{label}</strong></div>)}</div><button onClick={importLetterboxdExport}><Icon name="external" size={18}/>Commencer l’import</button><small>Cette étape complète n’est nécessaire qu’une fois. Aucun fichier n’est envoyé à nos serveurs.</small>{error&&<div className="syncError" role="alert"><Icon name="alert" size={17}/><span><strong>L’import s’est interrompu</strong>{error}</span></div>}</section>:<><section className="syncSummary"><div><span className="bigCount">{selectable.length}</span><div><h1>film{selectable.length>1?"s":""} à importer</h1><p>{selectable.length?"Choisis ceux à ajouter à Letterboxd.":"Actualise pour chercher les nouveaux films."}</p></div></div><button onClick={refresh} disabled={loading}><Icon name="sync" size={18}/>{loading?"Actualisation…":"Actualiser"}</button>{error&&<div className="syncError" role="alert"><Icon name="alert" size={17}/><span><strong>Un problème est survenu</strong>{error}</span></div>}{notice&&<div className="syncNotice"><Icon name="alert" size={17}/><span>{notice}</span></div>}{importReady&&<div className="importReady"><div><strong>Fichier prêt</strong><span>Le téléchargement est terminé.</span></div><button onClick={openLetterboxdImport}><Icon name="external" size={17}/>Ouvrir dans le navigateur</button></div>}</section>
